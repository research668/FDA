...
#PAB tree
def PAB_tree(X_PAB,y_PAB,saa_decision):
    K,N,f = X_PAB.shape
    selected_list = []
    weight_list = []
    ...
    y_proxy = np.array(y_proxy).astype('float64')
    data = np.array(data).astype('float64')
    y_true = np.array(y_true).astype('float64')
    
    data_train_gold_total = xgb.DMatrix(data,label = y_true)
    data_train_proxy_total = xgb.DMatrix(data,label = y_proxy)

 
    list_train = list(np.random.choice(range(data.shape[0]),
                                  size = int(0.8*data.shape[0]),replace=False))
    list_validataion = [i for i in range(data.shape[0]) if i not in list_train]
    data_train_gold = xgb.DMatrix(data[list_train,:],label = y_true[list_train])
    data_train_proxy = xgb.DMatrix(data[list_train,:],label = y_proxy[list_train])

    

    data_validation = xgb.DMatrix(data[list_validataion,:])
    label_validation = y_true[list_validataion]

    ##设置总迭代次数
    n_round = 200
        
    ##设置gold和proxy数据集上的迭代比例调整范围
    ratio_range = range(1,10)
        
  
    ##PAB
    ##设置参数调整的范围
    max_depth_range = np.array(range(2,7))
    eta_range = [0.1,0.15,0.2]
    min_child_weight_range = np.array(range(1,5))
        
    ##初始化最优参数
    best_performance_round1 = 0
    best_performance_depth = 0
    best_performance_eta = 0
    best_performance_child = 0
    best_performance = 10**31

    ##最优参数寻找开始
    for round_temp in ratio_range:
        n_round_1_temp = int((round_temp/20)*n_round)    ##先训次数
        n_round_2_temp = n_round - n_round_1_temp
        for depth in max_depth_range:
            for eta_temp in eta_range:
                for child in min_child_weight_range:
                    param_p1_temp = {'max_depth':depth, 'eta':eta_temp, 'objective':'reg:squarederror','eval_metric':'rmse','min_child_weight':child}
                    param_p2_temp = {'max_depth':depth, 'eta':eta_temp, 'objective':'reg:squarederror','eval_metric':'rmse','min_child_weight':child}
                    xgb_model_Proxy_temp = xgb.train(param_p1_temp, data_train_proxy,n_round_1_temp)
                    xgb_model_Proxy_first  = xgb.train(param_p2_temp, data_train_gold ,xgb_model = xgb_model_Proxy_temp,num_boost_round = n_round_2_temp)
                    pred_Proxy_first_vali = xgb_model_Proxy_first.predict(data_validation)
                   
                    ##预测结果调整
                    pred_Proxy_first_vali = np.maximum(np.array(pred_Proxy_first_vali),0)
                    
                    ##这里使用sMAPE来寻找最优参数，可自行调整为其他指标
                    sMAPE_PAB_temp_vali = np.mean((pred_Proxy_first_vali - label_validation)**2)
                    if sMAPE_PAB_temp_vali < best_performance:
                        best_performance_depth = depth
                        best_performance_eta = eta_temp
                        best_performance_child = child
                        best_performance_round1 = n_round_1_temp
                        best_performance = sMAPE_PAB_temp_vali 
    param_p1 = {'max_depth':best_performance_depth, 'eta':best_performance_eta, 'objective':'reg:squarederror','eval_metric':'rmse','min_child_weight':best_performance_child}  
    param_p2 = {'max_depth':best_performance_depth, 'eta':best_performance_eta, 'objective':'reg:squarederror','eval_metric':'rmse','min_child_weight':best_performance_child}  
    nround_2_proxy_first = n_round - best_performance_round1
    xgb_model_Proxy_temp = xgb.train(param_p1, data_train_proxy_total,best_performance_round1)
    xgb_model_Proxy_first  = xgb.train(param_p2, data_train_gold_total ,xgb_model = xgb_model_Proxy_temp,num_boost_round = nround_2_proxy_first)

    
    # 在返回结果之前清理所有局部变量
    for var in list(locals().keys()):
        if var not in ['xgb_model_Proxy_first']:  # 保留结果变量
            del locals()[var]
    
    return  xgb_model_Proxy_first
